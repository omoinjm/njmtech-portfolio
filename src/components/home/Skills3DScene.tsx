...
const loadFaceImage = (src: string) =>
  new Promise<HTMLImageElement>((resolve, reject) => {
    ...
    image.onerror = () =>
      reject(new Error(`Unable to load skills die icon texture: ${src}`));
    image.src = src;
  });
...
const createBaseFaceTexture = (faceColor: string) => {
  ...
  if (!context) {
    throw new Error("Unable to create skills die texture.");
  }
...
const createDecalTexture = async (face: DieFace) => {
  ...
  const context = canvas.getContext("2d");
  if (!context) {
    throw new Error("Unable to create skills die decal texture.");
  }
...
const createDie = async (config: DieConfig): Promise<DieRuntime> => {
  ...
  const decalTextures = await Promise.all(
    config.faces.map((face) => createDecalTexture(face)),
  );
...
export const Skills3DScene = () => {
  ...
  useEffect(() => {
    ...
    const ambientLight = new THREE.AmbientLight("#f5f3ff", 1.2);
    const hemisphereLight = new THREE.HemisphereLight("#ede9fe", "#1e1b4b", 1.2);
    const directionalLight = new THREE.DirectionalLight("#ffffff", 2.2);
    directionalLight.position.set(3, 5, 4);
    directionalLight.castShadow = true;

    const pointLightA = new THREE.PointLight("#7c3aed", 2.0);
    pointLightA.position.set(-3, -2, 4);

    const pointLightB = new THREE.PointLight("#a78bfa", 1.8);
    pointLightB.position.set(3, 2, -2);

    const pointLightC = new THREE.PointLight("#818cf8", 1.5);
    pointLightC.position.set(0, 3, 3);
    ...
    void Promise.all(DICE.map((dieConfig) => createDie(dieConfig)))
      ...
      .catch((error: unknown) => {
        console.error("Failed to load skills die textures:", error);
      });
