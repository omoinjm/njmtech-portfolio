...
export async function seed() {
	try {
		await sql.transaction((tx) => [
			//#region Menu
			tx`CREATE TABLE IF NOT EXISTS nav_menu (
			...
			tx`INSERT INTO nav_menu (name, icon, route_url, is_active, sort_order) VALUES ('Projects', 'bi bi-window', '/projects', true, 1) ON CONFLICT (route_url) DO NOTHING`,
			tx`INSERT INTO nav_menu (name, icon, route_url, is_active, sort_order) VALUES ('Contact', 'bi bi-telephone', '/contact', true, 2) ON CONFLICT (route_url) DO NOTHING`,
			tx`INSERT INTO nav_menu (name, icon, route_url, is_active, sort_order) VALUES ('Subscribe', 'bi bi-pen', '/subscribe', true, 3) ON CONFLICT (route_url) DO NOTHING`,
			//#endregion

			//#region Footer
			tx`CREATE TABLE IF NOT EXISTS nav_footer (
				id SERIAL PRIMARY KEY,
				name VARCHAR(255) NOT NULL,
				icon VARCHAR(255),
				route_url VARCHAR(255) NOT NULL UNIQUE,
			...
			tx`CREATE TABLE IF NOT EXISTS mail_template (
				id SERIAL PRIMARY KEY,
